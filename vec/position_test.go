...
func TestIntVector_Check(t *testing.T) {
	...
	tests := []testIntVec{
		testIntVec{"single", uneven, func(v int) bool { return v == 3 }, []bool{T, F, F, F, F, F, F, F, F, F}, nil},
		testIntVec{"multiple", bins, func(v int) bool { return v == 0 }, []bool{T, F, T, T, T, F, F, F, T}, nil},
		testIntVec{"none", uniq, func(v int) bool { return v > 30 }, []bool{F, F, F, F, F, F}, nil},
		testIntVec{"error", uniq, func(v string) bool { return v > "A" }, nil, fmt.Errorf("wrong function, expected: `func(int) bool`, got `func(string) bool`")},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestStrVector_Check(t *testing.T) {
	...
	tests := []testStrVec{
		testStrVec{"single", uniq, func(v string) bool { return v == "R" }, []bool{F, F, T, F, F, F}, nil},
		testStrVec{"multiple", bins, func(v string) bool { return v == "0" }, []bool{T, F, T, T, T, F, F, F, T}, nil},
		testStrVec{"none", bins, func(v string) bool { return v > "z" }, []bool{F, F, F, F, F, F, F, F, F}, nil},
		testStrVec{"error", uniq, func(v string) int { return 0 }, nil, fmt.Errorf("wrong function, expected: `func(string) bool`, got `func(string) int`")},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := tt.v.Check(tt.args)
...
func TestIntVector_Filter(t *testing.T) {
	...
	errW := StrVector{err: fmt.Errorf(
		"error in Check: %w",
		fmt.Errorf("wrong function, expected: `func(int) bool`, got `func(string) int`"),
	)}

	tests := []testIntVec{
		testIntVec{"between", uniq, func(v int) bool { return v > -1 && v < 3 }, betw},
		testIntVec{"higher", uniq, func(v int) bool { return v >= 6 }, last},
		testIntVec{"equals", bins, func(v int) bool { return v == 0 }, binsW},
		testIntVec{"error", bins, func(a string) int { return 2 }, errW},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := tt.v.Filter(tt.args); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("IntVector.Filter() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func TestStrVector_Filter(t *testing.T) {
	...
	errW := StrVector{err: fmt.Errorf(
		"error in Check: %w",
		fmt.Errorf("wrong function, expected: `func(string) bool`, got `func(int) string`"),
	)}

	tests := []testStrVec{
		testStrVec{"between", uniq, func(v string) bool { return v > "a" && v < "bb" }, smaller},
		testStrVec{"larger", uniq, func(v string) bool { return v > "p" }, last},
		testStrVec{"equals", bins, func(v string) bool { return v == "1" }, binsW},
		testStrVec{"isin", uneven, Isin([]string{"a", "b", "z"}), isin},
		testStrVec{"error", bins, func(a int) string { return "nope" }, errW},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := tt.v.Filter(tt.args); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("StrVector.Filter() = %v, want %v", got, tt.want)
			}
		})
	}
}
